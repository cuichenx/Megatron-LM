...
def test_training_entrypoint_initializes_services_after_config(entrypoint: str):
    ...
    runtime_calls = _calls(tree, "initialize_training_globals")
    pretrain_calls = _calls(tree, "pretrain")
    assert len(parse_calls) == len(config_calls) == len(runtime_calls) == 1
    assert parse_calls[0].lineno < config_calls[0].lineno < runtime_calls[0].lineno
    assert pretrain_calls
    assert all(call.lineno > runtime_calls[0].lineno for call in pretrain_calls)
    assert not _calls(tree, "initialize_runtime_services")
...
def test_pretrain_registers_config_without_reinitializing_services():
    ...
    register = _calls(pretrain, "set_full_config")
    distributed = _calls(pretrain, "initialize_megatron")
    assert len(register) == 1
    assert register[0].lineno < min(call.lineno for call in distributed)
    assert register[0].args[0].id == "cfg_container"
