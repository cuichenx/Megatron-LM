...
| --moe-router-use-fp32 | Use FP32 precision for routing computation and expert output weighted averaging. This can improve numerical stability, particularly when using a large number of experts (>=32). The throughput impact should be negligible when used with permute fusion. |
...
| --moe-use-legacy-grouped-gemm | Use legacy GroupedMLP rather than TEGroupedMLP. Note: The legacy one will be deprecatedin v0.13. |
